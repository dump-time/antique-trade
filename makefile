OUTPUT_DIR = ./dist
OUTPUT_BIN = antique-trade.out
LOG_FILE = $(OUTPUT_DIR)/out.log
PID_FILE = /tmp/antique-trade.pid
...
.PHONY: start
start: stop build
	@nohup $(OUTPUT_DIR)/$(OUTPUT_BIN) -d --config "./data/config.yml" > $(LOG_FILE) 2>&1 & echo $$! > $(PID_FILE)
	@echo "Server started, all file is in dist directory"

.PHONY: run
run: build
	@$(OUTPUT_DIR)/$(OUTPUT_BIN) --config "./data/config.yml"
